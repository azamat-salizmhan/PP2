...
BEGIN
    FOR i IN 1 .. array_length(p_names, 1) LOOP
        -- Валидация: Телефон нөмірі 10 цифрдан кем болмауы керек (мысал ретінде)
        IF length(p_phones[i]) >= 10 THEN
            INSERT INTO contacts(name, phone) VALUES(p_names[i], p_phones[i])
            ON CONFLICT (name) DO UPDATE SET phone = p_phones[i];
        ELSE
            RAISE NOTICE 'Қате нөмір өткізілді: %', p_phones[i];
        END IF;
    END LOOP;
END;
$$;
